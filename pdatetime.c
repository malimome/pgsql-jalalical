 ...
 *
 * pdatetime.c
 *	  Support functions for pdate/time types.
 *
 * IDENTIFICATION
...
static int	P_DecodeDate(char *str, int fmask, int *tmask, bool *is2digits,
					   struct pg_tm * tm);
static int	P_ValidateDate(int fmask, bool is2digits, bool bc,
						 struct pg_tm * tm);
static void TrimTrailingZeros(char *str);

// Needed variables
const int	pday_tab[2][13] =
{
	{31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29, 0},
	{31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 30, 0},
};

char	   *months[] = {"Far", "Ord", "Kho", "Tir", "Mor", "Sha",
"Meh", "Aba", "Aza", "Dey", "Bah", "Esf", NULL};
...
/*	text, token, lexval */
	...
	{"jd", UNITS, DTK_JULIAN},
	{"meh", MONTH, 7},
	{"persian", UNITS, DTK_JULIAN}, //MOHSEN
	{"mehr", MONTH, 7},
	{"sha", MONTH, 6},
	{"shahrivar", MONTH, 6},
...
int p_days_in_month[12] = {31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29};
// Converts a date to number of days after Iran revolution 57/11/22, This is stored in database
int
date2p(int y, int m, int d)
{
	long p_day_no;
  int i;
  ...
  p_day_no = 365*y + (y/33)*8 + (y%33+3)/4;
  for (i=0; i < m; ++i)
     p_day_no += p_days_in_month[i];

  p_day_no += d;
	return (int)(p_day_no - PEPOCH);
}	/* date2p() */

// Converts number of days from Iran Revolution 57/11/22, to date.
void
p2date(int jd, int *py, int *pm, int *pd)
{
	int i;
	int p_day_no = jd + PEPOCH;
  int p_np = p_day_no / 12053;
  p_day_no %= 12053;
 
  *py = 979+33*p_np+4*(p_day_no/1461);
  p_day_no %= 1461;
...
// Converts date to number of day in Julain system.
int
...
/*
 * p2day - convert Persian date to day-of-week (0..6 == Sun..Sat)
 *
 * Note: various places use the locution p2day(date - 1) to produce a
...
void pdate2date(struct pg_tm ptm, struct pg_tm *tm)
{
	int gy, gm, gd;
  int jy, jm, jd;
  ...
  gd = g_day_no+1;
	...
	strncpy((void *)tm, (void *)&ptm, sizeof(struct pg_tm));
  tm->tm_year = gy;
  tm->tm_mon = gm;
  tm->tm_mday = gd;
	return;
...
void date2pdate(struct pg_tm tm, struct pg_tm *ptm)
{
  int gy, gm, gd;
  int jy, jm, jd;
  long g_day_no, j_day_no;
  ...
  for (i = 0; i < 11 && j_day_no >= p_days_in_month[i]; ++i) {
     j_day_no -= p_days_in_month[i];
  }
  jm = i+1;
  jd = j_day_no+1;
 
	strncpy((void *)ptm, (void *)&tm, sizeof(struct pg_tm));
	ptm->tm_year = jy;
	ptm->tm_mon = jm;
	ptm->tm_mday = jd;
	return;
...
/*
 * P_GetCurrentDateTime()
 *
 * Get the Persian transaction start time ("now()") broken down as a struct pg_tm.
 */
void
P_GetCurrentDateTime(struct pg_tm * ptm)
{
	int			tz;
	fsec_t		fsec;
	struct pg_tm tm;
...
void
P_GetCurrentTimeUsec(struct pg_tm * ptm, fsec_t *fsec, int *tzp)
{
	int			tz;
	struct pg_tm tm;
 ...
 */
int
P_DecodeDateTime(char **field, int *ftype, int nf,
			   int *dtype, struct pg_tm * tm, fsec_t *fsec, int *tzp)
{
	...
	*fsec = 0;
		...
		switch (ftype[i])
		{
			case DTK_DATE:
				...
				if (ptype == DTK_JULIAN)
				{
					...
					p2date(val, &tm->tm_year, &tm->tm_mon, &tm->tm_mday);
				...
				{
					dterr = P_DecodeDate(field[i], fmask,
									   &tmask, &is2digits, tm);
					if (dterr)
						return dterr;
				}
					...
					case RESERV:
						switch (val)
						{
							...
							case DTK_NOW:
								tmask = (DTK_DATE_M | DTK_TIME_M | DTK_M(TZ));
								*dtype = DTK_DATE;
								P_GetCurrentTimeUsec(tm, fsec, tzp);
								break;
							...
							case DTK_YESTERDAY:
								...
								*dtype = DTK_DATE;
								...
								p2date(date2p(tm->tm_year, tm->tm_mon, tm->tm_mday) - 1,
	...
	dterr = P_ValidateDate(fmask, is2digits, bc, tm);
	if (dterr)
		return dterr;
	...
	if (*dtype == DTK_DATE)
	{
		...
		if (namedTz != NULL)
		{
			...
			*tzp = P_DetermineTimeZoneOffset(tm, namedTz);
		}
		...
		if (tzp != NULL && !(fmask & DTK_M(TZ)))
		{
			...
			*tzp = P_DetermineTimeZoneOffset(tm, session_timezone);
		}
	}
 ...
 */
int
P_DetermineTimeZoneOffset(struct pg_tm * tm, pg_tz *tzp)
{
	int			date,
				sec;
	pg_time_t	day,
	...
	if (!IS_VALID_PERSIAN(tm->tm_year, tm->tm_mon, tm->tm_mday))
		goto overflow;
	date = date2p(tm->tm_year, tm->tm_mon, tm->tm_mday) - UNIX_EPOCH_JDATE;
			   ...
			   int *dtype, struct pg_tm * tm, fsec_t *fsec, int *tzp)
{
	...
	*fsec = 0;
					...
					case RESERV:
						switch (val)
						{
							...
							case DTK_NOW:
								tmask = DTK_TIME_M;
								*dtype = DTK_TIME;
								P_GetCurrentTimeUsec(tm, fsec, NULL);
								break;
	...
	if (tzp != NULL && !(fmask & DTK_M(TZ)))
	{
		...
		*tzp = P_DetermineTimeZoneOffset(tmp, session_timezone);
 ...
 */
static int
P_DecodeDate(char *str, int fmask, int *tmask, bool *is2digits,
		   struct pg_tm * tm)
{
	fsec_t		fsec;
	int			nf = 0;
	...
	*tmask = 0;
	...
	/* validation of the field values must wait until P_ValidateDate() */
 ...
 */
static int
P_ValidateDate(int fmask, bool is2digits, bool bc, struct pg_tm * tm)
{
	if (fmask & DTK_M(YEAR))
	{
...
// MOHSEN
	...
	if (fmask & DTK_M(DOY))
	{
		p2date(date2p(tm->tm_year, 1, 1) + tm->tm_yday - 1,
			   &tm->tm_year, &tm->tm_mon, &tm->tm_mday);
	}
...
EncodeDateTime(struct pg_tm * tm, fsec_t fsec, int *tzp, char **tzn, int style, char *str)
{
	...
	switch (style)
	{
		...
		default:
			...
			day = date2p(tm->tm_year, tm->tm_mon, tm->tm_mday);
			tm->tm_wday = p2day(day);
...
CheckDateTokenTables(void)
{
	...
	Assert(UNIX_EPOCH_JDATE == date2p(1970, 1, 1));
	Assert(POSTGRES_EPOCH_JDATE == date2p(2000, 1, 1));
