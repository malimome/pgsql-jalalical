 ...
 *
 * pdatetime.h
 *	  Definitions for the pdate/time and other pdate/time support code.
 *	  The support code is shared with other pdate data types,
 *	   including ptimestamp, ptimestamptz.
 *
 *  By Mohsen Alimomeni
 *
 *-------------------------------------------------------------------------
 ...
 *
 * These strings are the defaults used to form output time str_Pings.
...
//#define p_isleap(y) (((y) % 4) == 0 && (((y) % 100) != 0 || ((y) % 400) == 0))

#define p_isleap(y) (((y) % 33) == 1 || ((y) % 33) == 5 || ((y) % 33) == 9 || \
									((y) % 33) == 13 || ((y) % 33) == 17 || ((y) % 33) == 22 || \
									((y) % 33) == 26 || ((y) % 33) == 30) 


/* Julian date support for date2p() and p2date()
 *
 * IS_VALID_PERSIAN checks the minimum date exactly, but is a bit sloppy
...
#define IS_VALID_PERSIAN(y,m,d) ((((y) > JULIAN_MINYEAR) \
  || (((y) == JULIAN_MINYEAR) && (((m) > JULIAN_MINMONTH) \
  || (((m) == JULIAN_MINMONTH) && ((d) >= JULIAN_MINDAY))))) \
 && ((y) < JULIAN_MAXYEAR))

/* Julian-date equivalents of Day 0 in Unix and Postgres reckoning */
#define UNIX_EPOCH_JDATE		2440588 /* == date2p(1970, 1, 1) */
#define POSTGRES_EPOCH_JDATE	2451545 /* == date2p(2000, 1, 1) */


/*
 * Datetime input parsing routines (ParseDateTime, P_DecodeDateTime, etc)
...
void P_GetCurrentDateTime(struct pg_tm * tm);
void GetCurrentTimeUsec(struct pg_tm * tm, fsec_t *fsec, int *tzp);
void P_GetCurrentTimeUsec(struct pg_tm * tm, fsec_t *fsec, int *tzp);
void p2date(int jd, int *year, int *month, int *day);
int	date2p(int year, int month, int day);
...
int P_DecodeDateTime(char **field, int *ftype,
			   int nf, int *dtype,
...
int	P_DetermineTimeZoneOffset(struct pg_tm * tm, pg_tz *tzp);
...
//void InstallTimeZoneAbbrevs(tzEntry *abbrevs, int n);
